...
export default function FinanciaPlay() {
  ...
  const availableLevels = userProfile ? getAvailableLevels(userProfile.knowledgeLevel) : [];
  ...
  function startGame(game) {
    ...
    if (game.type === "quiz_mc") {
      setGameState({ phase: "playing", qIdx: 0, answers: [], score: 0, feedback: null });
    } else if (game.type === "true_false") {
      setGameState({ phase: "playing", sIdx: 0, answers: [], score: 0, feedback: null });
      ...
      const rights = [...game.pairs.map((p, i) => ({ id: i, text: p.right }))].sort(() => Math.random() - 0.5);
      setGameState({ phase: "playing", lefts, rights, selected: null, matched: {}, errors: 0, score: 0 });
    }
  ...
  function finishGame(score, maxScore) {
    ...
    setGameState(gs => ({ ...gs, phase: "summary", finalScore: score, maxScore, xpGained }));
  }
  ...
  async function fetchMiniLesson(topicId, levelId) {
    ...
    const allTopics = [...CURRICULUM.level1.topics, ...CURRICULUM.level2.topics, ...CURRICULUM.level3.topics];
    const topic = allTopics.find(t => t.id === topicId);
    const levelKey = levelId === "L1" ? "level1" : levelId === "L2" ? "level2" : "level3";
...
El lenguaje debe ser cercano, motivador y evitar jerga sin explicar. Si el nivel es L3 (experto), usa terminología técnica precisa con referencias a estándares internacionales (CFA, GARP, Basilea, etc.).`;
    ...
    try {
      ...
      setAiContent({ emoji: "📚", title: topic?.name, hook: "Cargando contenido educativo...", concept: "Hubo un problema al cargar la lección. Intenta de nuevo.", example: "", keyFact: "", challenge: "" });
    }
    setAiLoading(false);
  }

  // ---- RENDER ----
  if (screen === "home") return <HomeScreen onStart={() => setScreen("placement")} />;
  if (screen === "placement") return <PlacementScreen
    idx={placementIdx} answers={placementAnswers}
    onAnswer={(ans) => {
  ...
  if (screen === "course") return <CourseScreen
    ...
    onRetest={() => { setPlacementIdx(0); setPlacementAnswers([]); setUserProfile(null); setScreen("placement"); }}
  />;
  if (screen === "game") return <GameScreen
    game={currentGame} gameState={gameState} timeLeft={timeLeft}
    setGameState={setGameState}
    onFinish={finishGame}
    onBack={() => { clearTimeout(timerRef.current); setTimeLeft(null); setScreen("course"); }}
  />;
  if (screen === "mini_lesson") return <MiniLessonScreen
    loading={aiLoading} content={aiContent}
    onBack={() => setScreen("course")}
    onPlayGame={() => {
      const levelKey = aiContent ? "L" + (CURRICULUM.level1.topics.find(t => t.id === miniLesson) ? "1" : CURRICULUM.level2.topics.find(t => t.id === miniLesson) ? "2" : "3") : null;
      setScreen("course");
    }}
  />;
...
function HomeScreen({ onStart }) {
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0a0f1e 0%, #0d1b3e 50%, #0a0f1e 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Georgia', serif", padding: "24px", position: "relative", overflow: "hidden" }}>
      {/* Decorative orbs */}
      <div style={{ position: "absolute", top: "10%", left: "5%", width: 300, height: 300, borderRadius: "50%", background: "radial-gradient(circle, rgba(0,200,150,0.15) 0%, transparent 70%)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", bottom: "10%", right: "5%", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(59,130,246,0.12) 0%, transparent 70%)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", top: "40%", right: "20%", width: 200, height: 200, borderRadius: "50%", background: "radial-gradient(circle, rgba(245,158,11,0.1) 0%, transparent 70%)", pointerEvents: "none" }} />

      <div style={{ textAlign: "center", zIndex: 1, maxWidth: 600 }}>
        <div style={{ fontSize: 72, marginBottom: 8 }}>📈</div>
        <h1 style={{ fontSize: "clamp(2.4rem, 6vw, 4rem)", fontWeight: 700, color: "#fff", margin: "0 0 8px", letterSpacing: "-2px", lineHeight: 1.1 }}>
          Financia<span style={{ color: "#00C896" }}>Play</span>
        </h1>
        <p style={{ color: "#94a3b8", fontSize: "1.15rem", marginBottom: 8, fontStyle: "italic" }}>
          De billetera a mercados de capitales — aprende jugando
        </p>
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginBottom: 32 }}>
          {["🌱 Fundamentos", "📊 Inversiones", "🏆 Derivados & Capitales"].map((t, i) => (
            <span key={i} style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 20, padding: "6px 14px", color: "#cbd5e1", fontSize: "0.85rem" }}>{t}</span>
          ))}
        </div>
        <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, padding: "20px 28px", marginBottom: 32, textAlign: "left" }}>
          <p style={{ color: "#e2e8f0", margin: 0, lineHeight: 1.7, fontSize: "0.95rem" }}>
            🎯 <strong style={{ color: "#00C896" }}>Test de diagnóstico</strong> adaptativo para ubicarte en tu nivel real<br />
            🎮 <strong style={{ color: "#3B82F6" }}>Juegos educativos</strong> dinámicos: quizzes, decisiones, retos<br />
            🤖 <strong style={{ color: "#F59E0B" }}>Lecciones con IA</strong> personalizadas para cada tema<br />
            🏆 <strong style={{ color: "#a78bfa" }}>3 niveles</strong> desde finanzas personales hasta derivados
          </p>
        </div>
        <button onClick={onStart} style={{ background: "linear-gradient(135deg, #00C896, #00a07a)", color: "#fff", border: "none", borderRadius: 14, padding: "16px 48px", fontSize: "1.15rem", fontWeight: 700, cursor: "pointer", letterSpacing: 0.5, boxShadow: "0 8px 32px rgba(0,200,150,0.4)", transition: "transform 0.15s, box-shadow 0.15s" }}
          onMouseOver={e => { e.target.style.transform = "scale(1.04)"; e.target.style.boxShadow = "0 12px 40px rgba(0,200,150,0.55)"; }}
          onMouseOut={e => { e.target.style.transform = "scale(1)"; e.target.style.boxShadow = "0 8px 32px rgba(0,200,150,0.4)"; }}>
          Comenzar Test →
        </button>
        <p style={{ color: "#475569", fontSize: "0.8rem", marginTop: 14 }}>~3 minutos · 12 preguntas de diagnóstico</p>
      </div>
    </div>
  );
}
...
function PlacementScreen({ idx, answers, onAnswer }) {
  ...
  const diffColor = q.level === 1 ? "#00C896" : q.level === 2 ? "#3B82F6" : "#F59E0B";
  const diffLabel = q.level === 1 ? "Básico" : q.level === 2 ? "Intermedio" : "Avanzado";

  return (
    <div style={{ minHeight: "100vh", background: "#080d1a", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Georgia', serif", padding: "20px" }}>
      <div style={{ width: "100%", maxWidth: 560 }}>
        {/* Progress */}
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <span style={{ color: "#64748b", fontSize: "0.85rem" }}>Test de Diagnóstico</span>
            <span style={{ color: "#94a3b8", fontSize: "0.85rem" }}>{idx + 1} / {total}</span>
          </div>
          <div style={{ height: 6, background: "#1e293b", borderRadius: 3, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${pct}%`, background: "linear-gradient(90deg, #00C896, #3B82F6)", borderRadius: 3, transition: "width 0.4s" }} />
          </div>
        </div>

        {/* Difficulty badge */}
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
          <span style={{ background: diffColor + "22", color: diffColor, border: `1px solid ${diffColor}44`, borderRadius: 20, padding: "4px 12px", fontSize: "0.78rem", fontWeight: 600 }}>{diffLabel}</span>
        </div>

        {/* Question card */}
        <div style={{ background: "linear-gradient(135deg, #0f172a, #1e293b)", border: "1px solid #334155", borderRadius: 20, padding: "32px", marginBottom: 24, boxShadow: "0 8px 32px rgba(0,0,0,0.4)" }}>
          <p style={{ color: "#f1f5f9", fontSize: "1.2rem", fontWeight: 600, lineHeight: 1.5, margin: 0 }}>{q.q}</p>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {q.opts.map((opt, i) => (
            <button key={i} onClick={() => onAnswer(i)}
              style={{ background: "rgba(255,255,255,0.04)", border: "1px solid #334155", borderRadius: 14, padding: "16px 20px", color: "#e2e8f0", fontSize: "0.95rem", textAlign: "left", cursor: "pointer", transition: "all 0.15s", display: "flex", alignItems: "center", gap: 12 }}
              onMouseOver={e => { e.currentTarget.style.background = "rgba(255,255,255,0.09)"; e.currentTarget.style.borderColor = diffColor; }}
              onMouseOut={e => { e.currentTarget.style.background = "rgba(255,255,255,0.04)"; e.currentTarget.style.borderColor = "#334155"; }}>
              <span style={{ width: 28, height: 28, borderRadius: "50%", background: "#1e293b", border: "1px solid #475569", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.8rem", color: "#64748b", flexShrink: 0 }}>
                {["A", "B", "C", "D"][i]}
              </span>
              {opt}
            </button>
...
function ResultScreen({ profile, onContinue }) {
  ...
  const level = CURRICULUM[levelKey];
  const messages = {
    1: { title: "¡Bienvenido, Aprendiz! 🌱", sub: "Comenzarás con los fundamentos — la base de toda fortuna", tip: "Dato: Warren Buffett empezó comprando acciones a los 11 años. Nunca es tarde (ni temprano)." },
    2: { title: "¡Ya tienes base sólida! 📊", sub: "Saltamos a inversiones y análisis de mercados", tip: "Dato: El S&P 500 ha retornado ~10% anual en promedio en los últimos 100 años. La historia favorece al paciente." },
    3: { title: "¡Nivel Experto detectado! 🏆", sub: "Directo a mercados de capitales, derivados y gestión institucional", tip: "Dato: El mercado global de derivados supera los $600 billones en valor nocional. Bienvenido a las grandes ligas." },
  };
  const msg = messages[kl];

  return (
    <div style={{ minHeight: "100vh", background: "#080d1a", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Georgia', serif", padding: "24px" }}>
      <div style={{ maxWidth: 520, width: "100%", textAlign: "center" }}>
        <div style={{ fontSize: 72, marginBottom: 16 }}>{level.badge}</div>
        <h2 style={{ color: "#f1f5f9", fontSize: "1.9rem", fontWeight: 700, margin: "0 0 10px" }}>{msg.title}</h2>
        <p style={{ color: "#94a3b8", fontSize: "1rem", marginBottom: 24 }}>{msg.sub}</p>

        <div style={{ background: `linear-gradient(135deg, ${level.color}18, ${level.color}08)`, border: `1px solid ${level.color}40`, borderRadius: 18, padding: "24px", marginBottom: 24 }}>
          <div style={{ color: level.color, fontSize: "1.3rem", fontWeight: 700, marginBottom: 8 }}>Nivel asignado: {level.name}</div>
          <div style={{ color: "#94a3b8", fontSize: "0.9rem", lineHeight: 1.6, marginBottom: 16 }}>{level.description}</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, textAlign: "left" }}>
            {level.topics.slice(0, 4).map(t => (
              <div key={t.id} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "center" }}>
                <span style={{ fontSize: "1.1rem" }}>{t.icon}</span>
                <span style={{ color: "#cbd5e1", fontSize: "0.82rem", fontWeight: 500 }}>{t.name}</span>
              </div>
            ))}
          </div>
        </div>

        <div style={{ background: "#1e293b", borderRadius: 12, padding: "14px 18px", marginBottom: 28, textAlign: "left" }}>
          <p style={{ color: "#64748b", fontSize: "0.78rem", margin: "0 0 4px", fontStyle: "italic" }}>💡 Dato motivador</p>
          <p style={{ color: "#94a3b8", fontSize: "0.88rem", margin: 0, lineHeight: 1.5 }}>{msg.tip}</p>
        </div>

        <button onClick={onContinue} style={{ background: `linear-gradient(135deg, ${level.color}, ${level.darkColor})`, color: "#fff", border: "none", borderRadius: 14, padding: "15px 42px", fontSize: "1.05rem", fontWeight: 700, cursor: "pointer", boxShadow: `0 8px 28px ${level.color}44` }}>
          Comenzar mi curso →
        </button>
      </div>
    </div>
...
// COURSE SCREEN
// ============================================================
function CourseScreen({ profile, availableLevels, onStartGame, onMiniLesson, onRetest }) {
  const [activeLevel, setActiveLevel] = useState(availableLevels[availableLevels.length - 1]);
  const [expandedTopic, setExpandedTopic] = useState(null);
  const levelColors = { L1: "#00C896", L2: "#3B82F6", L3: "#F59E0B" };
  const levelData = { L1: CURRICULUM.level1, L2: CURRICULUM.level2, L3: CURRICULUM.level3 };
  ...
  const completedCount = curGames.filter(g => profile.completedGames.includes(g.id)).length;
  const color = levelColors[activeLevel];

  return (
    <div style={{ minHeight: "100vh", background: "#080d1a", fontFamily: "'Georgia', serif", padding: "0 0 80px" }}>
      {/* Header */}
      <div style={{ background: "linear-gradient(180deg, #0d1b3e 0%, #080d1a 100%)", borderBottom: "1px solid #1e293b", padding: "20px 24px 0" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <div>
              <h1 style={{ color: "#f1f5f9", fontSize: "1.4rem", fontWeight: 700, margin: 0 }}>FinanciaPlay 📈</h1>
              <p style={{ color: "#64748b", fontSize: "0.82rem", margin: "2px 0 0" }}>Tu camino al dominio financiero</p>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ background: "#1e293b", borderRadius: 12, padding: "8px 16px", textAlign: "center" }}>
                <div style={{ color: "#F59E0B", fontWeight: 700, fontSize: "1rem" }}>⚡ {profile.xp}</div>
                <div style={{ color: "#64748b", fontSize: "0.72rem" }}>XP Total</div>
              </div>
              <button onClick={onRetest} style={{ background: "transparent", border: "1px solid #334155", color: "#64748b", borderRadius: 10, padding: "8px 12px", cursor: "pointer", fontSize: "0.8rem" }}>🔄 Re-test</button>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {availableLevels.map(lid => {
              ...
              return (
                <button key={lid} onClick={() => setActiveLevel(lid)}
                  style={{ flex: 1, padding: "10px 8px", background: isActive ? c + "18" : "transparent", border: `1px solid ${isActive ? c + "60" : "transparent"}`, borderBottom: isActive ? `2px solid ${c}` : "2px solid transparent", borderRadius: "10px 10px 0 0", color: isActive ? c : "#64748b", fontSize: "0.82rem", fontWeight: isActive ? 700 : 400, cursor: "pointer", transition: "all 0.2s", textAlign: "center" }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "24px 20px" }}>
        {/* Level progress */}
        <div style={{ background: `linear-gradient(135deg, ${color}14, ${color}06)`, border: `1px solid ${color}30`, borderRadius: 18, padding: "20px 24px", marginBottom: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
            <div>
              <h2 style={{ color: "#f1f5f9", margin: 0, fontSize: "1.2rem", fontWeight: 700 }}>{curLevel.name}</h2>
              <p style={{ color: "#64748b", margin: "4px 0 0", fontSize: "0.85rem" }}>{curLevel.description}</p>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ color: color, fontWeight: 700 }}>{completedCount}/{curGames.length}</div>
              <div style={{ color: "#64748b", fontSize: "0.75rem" }}>juegos</div>
            </div>
          </div>
          <div style={{ height: 8, background: "#1e293b", borderRadius: 4, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${curGames.length > 0 ? (completedCount / curGames.length) * 100 : 0}%`, background: `linear-gradient(90deg, ${color}, ${color}aa)`, borderRadius: 4, transition: "width 0.5s" }} />
          </div>
        </div>

        {/* Topics */}
        <h3 style={{ color: "#94a3b8", fontSize: "0.85rem", fontWeight: 600, letterSpacing: 1, textTransform: "uppercase", marginBottom: 14 }}>📚 Temas del nivel</h3>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 10, marginBottom: 28 }}>
          {curLevel.topics.map(topic => (
            <button key={topic.id} onClick={() => onMiniLesson(topic.id, activeLevel)}
              style={{ background: "rgba(255,255,255,0.04)", border: "1px solid #1e293b", borderRadius: 14, padding: "14px", textAlign: "left", cursor: "pointer", transition: "all 0.2s", display: "flex", alignItems: "flex-start", gap: 10 }}
              onMouseOver={e => { e.currentTarget.style.background = "rgba(255,255,255,0.08)"; e.currentTarget.style.borderColor = color + "60"; }}
              onMouseOut={e => { e.currentTarget.style.background = "rgba(255,255,255,0.04)"; e.currentTarget.style.borderColor = "#1e293b"; }}>
              <span style={{ fontSize: "1.4rem", flexShrink: 0 }}>{topic.icon}</span>
              <div>
                <div style={{ color: "#e2e8f0", fontSize: "0.82rem", fontWeight: 600, marginBottom: 2 }}>{topic.name}</div>
                <div style={{ color: "#64748b", fontSize: "0.72rem", lineHeight: 1.3 }}>{topic.desc}</div>
              </div>
            </button>
          ))}
        </div>
        ...
        {/* Games */}
        <h3 style={{ color: "#94a3b8", fontSize: "0.85rem", fontWeight: 600, letterSpacing: 1, textTransform: "uppercase", marginBottom: 14 }}>🎮 Juegos educativos</h3>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {curGames.map((game, idx) => {
            const done = profile.completedGames.includes(game.id);
            const typeLabels = { quiz_mc: "Quiz", true_false: "V/F", categorize: "Clasifica", decision: "Decisión", match: "Conecta" };
            const typeColors = { quiz_mc: "#3B82F6", true_false: "#8B5CF6", categorize: "#F59E0B", decision: "#EF4444", match: "#10B981" };
            return (
              <div key={game.id} style={{ background: done ? "rgba(0,200,150,0.06)" : "rgba(255,255,255,0.04)", border: `1px solid ${done ? "#00C89640" : "#1e293b"}`, borderRadius: 16, padding: "16px 20px", display: "flex", alignItems: "center", gap: 16, transition: "all 0.2s" }}>
                <div style={{ fontSize: "1.8rem", flexShrink: 0 }}>{game.icon}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <span style={{ color: "#e2e8f0", fontWeight: 600, fontSize: "0.95rem" }}>{game.title}</span>
                    <span style={{ background: (typeColors[game.type] || "#64748b") + "22", color: typeColors[game.type] || "#64748b", fontSize: "0.72rem", padding: "2px 8px", borderRadius: 10, fontWeight: 600 }}>{typeLabels[game.type]}</span>
                    {done && <span style={{ color: "#00C896", fontSize: "0.75rem" }}>✓ Completado</span>}
                  </div>
                  <div style={{ display: "flex", gap: 14 }}>
                    <span style={{ color: "#64748b", fontSize: "0.78rem" }}>⏱ {game.timeSec}s</span>
                    <span style={{ color: "#F59E0B", fontSize: "0.78rem" }}>⚡ {game.points} pts</span>
                  </div>
                </div>
                <button onClick={() => onStartGame(game)}
                  style={{ background: done ? "#1e293b" : `linear-gradient(135deg, ${color}, ${levelColors[activeLevel]}aa)`, color: done ? "#64748b" : "#fff", border: "none", borderRadius: 12, padding: "10px 20px", fontSize: "0.88rem", fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap", boxShadow: done ? "none" : `0 4px 16px ${color}30` }}>
                  {done ? "Repetir" : "Jugar →"}
                </button>
              </div>
            );
...
// GAME SCREEN
// ============================================================
function GameScreen({ game, gameState, timeLeft, setGameState, onFinish, onBack }) {
  if (!game || !gameState) return null;
  const color = game.id.startsWith("L1") ? "#00C896" : game.id.startsWith("L2") ? "#3B82F6" : "#F59E0B";
  const timerPct = (timeLeft / game.timeSec) * 100;
  const timerColor = timerPct > 40 ? "#00C896" : timerPct > 15 ? "#F59E0B" : "#EF4444";
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#080d1a", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Georgia', serif", padding: "24px" }}>
        <div style={{ maxWidth: 460, width: "100%", textAlign: "center" }}>
          ...
          <h2 style={{ color: "#f1f5f9", fontSize: "1.7rem", fontWeight: 700, margin: "0 0 8px" }}>
            {pct >= 80 ? "¡Excelente!" : pct >= 50 ? "¡Bien hecho!" : "¡Sigue practicando!"}
          </h2>
          {gameState.timeUp && <p style={{ color: "#EF4444", fontSize: "0.9rem" }}>Se acabó el tiempo</p>}
          <div style={{ background: `linear-gradient(135deg, ${color}18, ${color}08)`, border: `1px solid ${color}40`, borderRadius: 18, padding: "24px", margin: "20px 0" }}>
            <div style={{ fontSize: "3rem", fontWeight: 800, color, margin: "0 0 4px" }}>{pct}%</div>
            <div style={{ color: "#94a3b8", marginBottom: 16 }}>{score} / {max} correctas</div>
            <div style={{ height: 10, background: "#1e293b", borderRadius: 5, overflow: "hidden", marginBottom: 16 }}>
              <div style={{ height: "100%", width: `${pct}%`, background: `linear-gradient(90deg, ${color}, ${color}88)`, borderRadius: 5 }} />
            </div>
            <div style={{ color: "#F59E0B", fontWeight: 700, fontSize: "1.1rem" }}>+{xp} XP ganados ⚡</div>
          </div>
          <button onClick={onBack} style={{ background: `linear-gradient(135deg, ${color}, ${color}aa)`, color: "#fff", border: "none", borderRadius: 14, padding: "14px 36px", fontSize: "1rem", fontWeight: 700, cursor: "pointer", marginRight: 12 }}>← Volver al curso</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080d1a", fontFamily: "'Georgia', serif", display: "flex", flexDirection: "column" }}>
      {/* Game header */}
      <div style={{ background: "#0d1b3e", borderBottom: "1px solid #1e293b", padding: "14px 20px", display: "flex", alignItems: "center", gap: 14, position: "sticky", top: 0, zIndex: 10 }}>
        <button onClick={onBack} style={{ background: "transparent", border: "1px solid #334155", color: "#94a3b8", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: "0.85rem" }}>✕</button>
        <div style={{ flex: 1 }}>
          <div style={{ color: "#f1f5f9", fontWeight: 600, fontSize: "0.9rem" }}>{game.title}</div>
        ...
        <div style={{ color: timerColor, fontWeight: 700, fontSize: "1.1rem", minWidth: 36, textAlign: "right" }}>
          {timeLeft !== null ? `${timeLeft}s` : ""}
        </div>
      </div>

      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", padding: "24px 20px", maxWidth: 600, width: "100%", margin: "0 auto" }}>
        {game.type === "quiz_mc" && <QuizGame game={game} gs={gameState} setGs={setGameState} onFinish={onFinish} color={color} />}
        {game.type === "true_false" && <TrueFalseGame game={game} gs={gameState} setGs={setGameState} onFinish={onFinish} color={color} />}
        {game.type === "categorize" && <CategorizeGame game={game} gs={gameState} setGs={setGameState} onFinish={onFinish} color={color} />}
        {game.type === "decision" && <DecisionGame game={game} gs={gameState} setGs={setGameState} onFinish={onFinish} color={color} />}
        {game.type === "match" && <MatchGame game={game} gs={gameState} setGs={setGameState} onFinish={onFinish} color={color} />}
      </div>
    </div>
  );
}
...
function QuizGame({ game, gs, setGs, onFinish, color }) {
  ...
  function answer(i) {
    ...
    setGs(s => ({ ...s, feedback: { chosen: i, correct }, score: s.score + (correct ? 1 : 0) }));
  }

  function next() {
    if (isLast) {
      const finalScore = gs.score + (gs.feedback?.correct ? 0 : 0); // already counted
      onFinish(gs.score + (gs.feedback?.correct ? 1 : 0) - (gs.answers.includes(true) ? 0 : 0), game.questions.length);
      // Simple: score was updated when answering
      const s = gs.score + (gs.feedback.correct ? 0 : 0);
      onFinish(gs.score + (gs.feedback?.correct ? 0 : 0), game.questions.length);
    } else {
      setGs(s => ({ ...s, qIdx: s.qIdx + 1, feedback: null, answers: [...s.answers, s.feedback?.correct] }));
    }
  }
  ...
  function answerFixed(i) {
    if (gs.feedback !== null) return;
    const correct = i === q.correct;
    setGs(s => ({ ...s, feedback: { chosen: i, correct }, score: s.score + (correct ? 1 : 0), answers: [...s.answers, correct] }));
  }
  ...
  return (
    <div style={{ width: "100%" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
        <span style={{ color: "#64748b", fontSize: "0.85rem" }}>Pregunta {gs.qIdx + 1} de {game.questions.length}</span>
        <span style={{ color: "#F59E0B", fontSize: "0.85rem" }}>⚡ {gs.score} pts</span>
      </div>
      <div style={{ background: "linear-gradient(135deg, #0f172a, #1e293b)", border: "1px solid #334155", borderRadius: 18, padding: "28px", marginBottom: 20 }}>
        <p style={{ color: "#f1f5f9", fontSize: "1.15rem", fontWeight: 600, lineHeight: 1.6, margin: 0 }}>{q.q}</p>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
        {q.opts.map((opt, i) => {
          let bg = "rgba(255,255,255,0.04)", border = "#334155", textColor = "#e2e8f0";
          if (gs.feedback) {
            if (i === q.correct) { bg = "rgba(0,200,150,0.15)"; border = "#00C896"; textColor = "#00C896"; }
            else if (i === gs.feedback.chosen && !gs.feedback.correct) { bg = "rgba(239,68,68,0.15)"; border = "#EF4444"; textColor = "#EF4444"; }
            else { bg = "rgba(255,255,255,0.02)"; border = "#1e293b"; textColor = "#475569"; }
          }
          return (
            <button key={i} onClick={() => answerFixed(i)} disabled={!!gs.feedback}
              style={{ background: bg, border: `1px solid ${border}`, borderRadius: 12, padding: "14px 18px", color: textColor, fontSize: "0.95rem", textAlign: "left", cursor: gs.feedback ? "default" : "pointer", transition: "all 0.2s", display: "flex", gap: 10, alignItems: "center" }}>
              <span style={{ width: 26, height: 26, borderRadius: "50%", background: "#1e293b", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.78rem", flexShrink: 0, color: "#64748b" }}>
                {i === q.correct && gs.feedback ? "✓" : i === gs.feedback?.chosen && !gs.feedback?.correct ? "✗" : ["A","B","C","D"][i]}
              </span>
              {opt}
            </button>
          );
      ...
      {gs.feedback && (
        <div style={{ background: gs.feedback.correct ? "rgba(0,200,150,0.1)" : "rgba(239,68,68,0.1)", border: `1px solid ${gs.feedback.correct ? "#00C89640" : "#EF444440"}`, borderRadius: 12, padding: "14px 18px", marginBottom: 16 }}>
          <p style={{ color: "#94a3b8", margin: 0, fontSize: "0.88rem", lineHeight: 1.5 }}>💡 {q.explain}</p>
        </div>
      )}
      {gs.feedback && (
        <button onClick={nextFixed} style={{ width: "100%", background: `linear-gradient(135deg, ${color}, ${color}aa)`, color: "#fff", border: "none", borderRadius: 12, padding: "14px", fontSize: "1rem", fontWeight: 700, cursor: "pointer" }}>
...
function TrueFalseGame({ game, gs, setGs, onFinish, color }) {
  ...
  function answer(bool) {
    ...
    setGs(prev => ({ ...prev, feedback: { chosen: bool, correct }, score: prev.score + (correct ? 1 : 0) }));
  }
  function next() {
  ...
  return (
    <div style={{ width: "100%" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
        <span style={{ color: "#64748b", fontSize: "0.85rem" }}>{gs.sIdx + 1} / {game.statements.length}</span>
        <span style={{ color: "#F59E0B", fontSize: "0.85rem" }}>⚡ {gs.score}</span>
      </div>
      <div style={{ background: "linear-gradient(135deg, #0f172a, #1e293b)", border: "1px solid #334155", borderRadius: 18, padding: "36px 28px", marginBottom: 28, textAlign: "center" }}>
        <p style={{ color: "#f1f5f9", fontSize: "1.2rem", fontWeight: 600, lineHeight: 1.6, margin: 0 }}>"{s.q}"</p>
      </div>
      {!gs.feedback && (
        <div style={{ display: "flex", gap: 16 }}>
          <button onClick={() => answer(true)} style={{ flex: 1, background: "rgba(0,200,150,0.1)", border: "2px solid #00C896", borderRadius: 14, padding: "20px", color: "#00C896", fontSize: "1.2rem", fontWeight: 700, cursor: "pointer" }}>✓ VERDADERO</button>
          <button onClick={() => answer(false)} style={{ flex: 1, background: "rgba(239,68,68,0.1)", border: "2px solid #EF4444", borderRadius: 14, padding: "20px", color: "#EF4444", fontSize: "1.2rem", fontWeight: 700, cursor: "pointer" }}>✗ FALSO</button>
        </div>
      )}
      {gs.feedback && (
        <>
          <div style={{ textAlign: "center", fontSize: "3rem", marginBottom: 12 }}>{gs.feedback.correct ? "✅" : "❌"}</div>
          <div style={{ background: gs.feedback.correct ? "rgba(0,200,150,0.1)" : "rgba(239,68,68,0.1)", borderRadius: 12, padding: "14px 18px", marginBottom: 16, textAlign: "center" }}>
            <div style={{ color: gs.feedback.correct ? "#00C896" : "#EF4444", fontWeight: 700, marginBottom: 6 }}>{gs.feedback.correct ? "¡Correcto!" : "Incorrecto"} — Era {s.answer ? "VERDADERO" : "FALSO"}</div>
            <p style={{ color: "#94a3b8", margin: 0, fontSize: "0.88rem" }}>💡 {s.explain}</p>
          </div>
          <button onClick={next} style={{ width: "100%", background: `linear-gradient(135deg, ${color}, ${color}aa)`, color: "#fff", border: "none", borderRadius: 12, padding: "14px", fontSize: "1rem", fontWeight: 700, cursor: "pointer" }}>
            {isLast ? "Ver resultados 🏆" : "Siguiente →"}
          </button>
        </>
      )}
...
function CategorizeGame({ game, gs, setGs, onFinish, color }) {
  ...
  function selectItem(label) { setSelected(selected === label ? null : label); }
  ...
  return (
    <div style={{ width: "100%" }}>
      <p style={{ color: "#94a3b8", fontSize: "0.9rem", marginBottom: 20, textAlign: "center" }}>Toca un ítem → luego toca la categoría</p>
      {/* Unplaced items */}
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 24, minHeight: 60, background: "rgba(255,255,255,0.03)", border: "1px dashed #334155", borderRadius: 14, padding: "12px" }}>
        {unplaced.map(item => (
          <button key={item.label} onClick={() => selectItem(item.label)}
            style={{ background: selected === item.label ? color + "33" : "rgba(255,255,255,0.06)", border: `1px solid ${selected === item.label ? color : "#334155"}`, borderRadius: 10, padding: "7px 14px", color: selected === item.label ? color : "#e2e8f0", fontSize: "0.85rem", cursor: "pointer", transition: "all 0.15s", fontWeight: selected === item.label ? 600 : 400 }}>
            {item.label}
          </button>
        ))}
        {unplaced.length === 0 && <span style={{ color: "#475569", fontSize: "0.85rem", alignSelf: "center", width: "100%", textAlign: "center" }}>¡Todos clasificados!</span>}
      </div>
      {/* Categories */}
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {game.categories.map(cat => {
          const placedHere = Object.entries(gs.placed).filter(([, v]) => v.catKey === cat.key);
          return (
            <div key={cat.key} onClick={() => selected && placeInCat(cat.key)}
              style={{ background: selected ? color + "0d" : "rgba(255,255,255,0.03)", border: `1px solid ${selected ? color + "60" : "#1e293b"}`, borderRadius: 14, padding: "14px 16px", cursor: selected ? "pointer" : "default", transition: "all 0.2s" }}>
              <div style={{ color: "#94a3b8", fontSize: "0.85rem", fontWeight: 600, marginBottom: 8 }}>{cat.label}</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {placedHere.map(([label, info]) => (
                  <span key={label} style={{ background: info.correct ? "rgba(0,200,150,0.15)" : "rgba(239,68,68,0.15)", border: `1px solid ${info.correct ? "#00C89660" : "#EF444460"}`, color: info.correct ? "#00C896" : "#EF4444", borderRadius: 8, padding: "4px 10px", fontSize: "0.8rem" }}>
                    {info.correct ? "✓" : "✗"} {label}
                  </span>
                ))}
                {placedHere.length === 0 && <span style={{ color: "#475569", fontSize: "0.78rem" }}>— vacío —</span>}
              </div>
            </div>
          );
        })}
      </div>
      <div style={{ textAlign: "right", marginTop: 14, color: "#64748b", fontSize: "0.82rem" }}>
        {Object.keys(gs.placed).length}/{game.items.length} clasificados · {gs.score} correctos
      </div>
    </div>
  );
}
...
function DecisionGame({ game, gs, setGs, onFinish, color }) {
  ...
  function choose(i) {
    ...
    setGs(prev => ({ ...prev, feedback: { chosen: i, correct }, score: prev.score + (correct ? 1 : 0) }));
  }
  function next() {
  ...
  return (
    <div style={{ width: "100%" }}>
      <div style={{ background: "linear-gradient(135deg, #1a1033, #0f1829)", border: "1px solid #4a3060", borderRadius: 18, padding: "28px", marginBottom: 24 }}>
        <div style={{ color: "#a78bfa", fontSize: "0.8rem", fontWeight: 600, marginBottom: 10 }}>🧠 ESCENARIO {gs.sIdx + 1}/{game.scenarios.length}</div>
        <p style={{ color: "#f1f5f9", fontSize: "1.1rem", fontWeight: 600, lineHeight: 1.6, margin: 0 }}>{sc.text}</p>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
        {sc.opts.map((opt, i) => {
          let bg = "rgba(167,139,250,0.06)", border = "#4a3060", tc = "#c4b5fd";
          if (gs.feedback) {
            if (i === sc.correct) { bg = "rgba(0,200,150,0.12)"; border = "#00C896"; tc = "#00C896"; }
            else if (i === gs.feedback.chosen) { bg = "rgba(239,68,68,0.12)"; border = "#EF4444"; tc = "#EF4444"; }
            else { bg = "rgba(255,255,255,0.02)"; border = "#1e293b"; tc = "#475569"; }
          }
          return (
            <button key={i} onClick={() => choose(i)} disabled={!!gs.feedback}
              style={{ background: bg, border: `1px solid ${border}`, borderRadius: 12, padding: "16px 20px", color: tc, fontSize: "0.95rem", textAlign: "left", cursor: gs.feedback ? "default" : "pointer", fontWeight: 500, transition: "all 0.2s" }}>
              {["A","B","C"][i]}. {opt}
            </button>
          );
        })}
      ...
      {gs.feedback && (
        <>
          <div style={{ background: "rgba(167,139,250,0.08)", border: "1px solid #4a306040", borderRadius: 12, padding: "14px 18px", marginBottom: 14 }}>
            <p style={{ color: "#94a3b8", margin: 0, fontSize: "0.88rem", lineHeight: 1.5 }}>💡 {sc.explain}</p>
          </div>
          <button onClick={next} style={{ width: "100%", background: `linear-gradient(135deg, ${color}, ${color}aa)`, color: "#fff", border: "none", borderRadius: 12, padding: "14px", fontSize: "1rem", fontWeight: 700, cursor: "pointer" }}>
...
function MatchGame({ game, gs, setGs, onFinish, color }) {
  ...
  function selectRight(id) {
    ...
    if (correct) {
      setGs(s => ({ ...s, matched: { ...s.matched, [s.selected]: true }, selected: null, score: s.score + 1 }));
    } else {
  ...
  return (
    <div style={{ width: "100%" }}>
      <p style={{ color: "#94a3b8", fontSize: "0.88rem", marginBottom: 20, textAlign: "center" }}>Selecciona un concepto → luego su definición</p>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ color: "#64748b", fontSize: "0.75rem", fontWeight: 600, textAlign: "center", marginBottom: 4 }}>CONCEPTO</div>
          {gs.lefts.map(item => {
            const matched = gs.matched[item.id] !== undefined;
            const selected = gs.selected === item.id;
            return (
              <button key={item.id} onClick={() => selectLeft(item.id)} disabled={matched}
                style={{ background: matched ? "rgba(0,200,150,0.12)" : selected ? color + "22" : "rgba(255,255,255,0.05)", border: `1px solid ${matched ? "#00C89650" : selected ? color : "#334155"}`, borderRadius: 10, padding: "12px", color: matched ? "#00C896" : selected ? color : "#e2e8f0", fontSize: "0.82rem", fontWeight: 500, cursor: matched ? "default" : "pointer", textAlign: "center", transition: "all 0.2s" }}>
                {matched ? "✓ " : ""}{item.text}
              </button>
            );
          })}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ color: "#64748b", fontSize: "0.75rem", fontWeight: 600, textAlign: "center", marginBottom: 4 }}>DEFINICIÓN</div>
          {gs.rights.map(item => {
            const matched = gs.matched[item.id] !== undefined;
            return (
              <button key={item.id} onClick={() => selectRight(item.id)} disabled={matched || gs.selected === null}
                style={{ background: matched ? "rgba(0,200,150,0.12)" : "rgba(255,255,255,0.05)", border: `1px solid ${matched ? "#00C89650" : "#334155"}`, borderRadius: 10, padding: "12px", color: matched ? "#00C896" : "#94a3b8", fontSize: "0.8rem", cursor: (matched || gs.selected === null) ? "default" : "pointer", textAlign: "center", transition: "all 0.2s" }}>
                {matched ? "✓ " : ""}{item.text}
              </button>
            );
          })}
      ...
      <div style={{ textAlign: "center", marginTop: 16, color: "#64748b", fontSize: "0.82rem" }}>
        {Object.keys(gs.matched).length}/{game.pairs.length} pareados · {gs.errors > 0 && `${gs.errors} errores`}
      </div>
    </div>
  );
}
...
function MiniLessonScreen({ loading, content, onBack }) {
  return (
    <div style={{ minHeight: "100vh", background: "#080d1a", fontFamily: "'Georgia', serif", padding: "24px 20px" }}>
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        <button onClick={onBack} style={{ background: "transparent", border: "1px solid #334155", color: "#64748b", borderRadius: 10, padding: "8px 16px", cursor: "pointer", marginBottom: 24, fontSize: "0.85rem" }}>← Volver</button>

        {loading && (
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            <div style={{ fontSize: 48, marginBottom: 16, animation: "pulse 1.5s infinite" }}>🤖</div>
            <p style={{ color: "#94a3b8" }}>Generando tu lección personalizada...</p>
            <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 16 }}>
              {[0,1,2].map(i => <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: "#3B82F6", opacity: 0.3 + i * 0.3 }} />)}
            </div>
          </div>
        )}
        ...
        {!loading && content && (
          <div>
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              <div style={{ fontSize: 56, marginBottom: 8 }}>{content.emoji}</div>
              <h2 style={{ color: "#f1f5f9", fontSize: "1.6rem", fontWeight: 700, margin: 0, lineHeight: 1.2 }}>{content.title}</h2>
            </div>

            {[
              { label: "🪝 ¿Lo sabías?", text: content.hook, color: "#F59E0B" },
              { label: "💡 El concepto", text: content.concept, color: "#3B82F6" },
              { label: "📊 Ejemplo real", text: content.example, color: "#00C896" },
              { label: "🔑 Dato clave", text: content.keyFact, color: "#a78bfa" },
            ].map((card, i) => card.text ? (
              <div key={i} style={{ background: card.color + "0d", border: `1px solid ${card.color}30`, borderRadius: 16, padding: "18px 20px", marginBottom: 14 }}>
                <div style={{ color: card.color, fontSize: "0.8rem", fontWeight: 700, marginBottom: 8, letterSpacing: 0.5 }}>{card.label}</div>
                <p style={{ color: "#e2e8f0", margin: 0, lineHeight: 1.7, fontSize: "0.92rem" }}>{card.text}</p>
              </div>
            ) : null)}

            {content.challenge && (
              <div style={{ background: "linear-gradient(135deg, #1a1033, #0f1829)", border: "1px solid #4a3060", borderRadius: 16, padding: "20px", marginTop: 8 }}>
                <div style={{ color: "#a78bfa", fontSize: "0.8rem", fontWeight: 700, marginBottom: 8 }}>🧠 MINI-RETO DE REFLEXIÓN</div>
                <p style={{ color: "#c4b5fd", margin: 0, lineHeight: 1.6, fontStyle: "italic" }}>{content.challenge}</p>
              </div>
            )}

            <button onClick={onBack} style={{ width: "100%", background: "linear-gradient(135deg, #3B82F6, #2563eb)", color: "#fff", border: "none", borderRadius: 14, padding: "15px", fontSize: "1rem", fontWeight: 700, cursor: "pointer", marginTop: 24 }}>
