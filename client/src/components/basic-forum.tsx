...
const Forum: React.FC = () => {
  ...
  const [showDeleteReplyDialog, setShowDeleteReplyDialog] = useState<{
    open: boolean;
    postId: number | null;
    replyId: number | null;
  }>({ open: false, postId: null, replyId: null });
  ...
  const confirmarEliminarPublicacion = () => {
    if (showDeletePostDialog.postId !== null) {
      const updatedPosts = posts.filter(
        (post) => post.id !== showDeletePostDialog.postId,
      );
  ...
  const defaultPosts: Post[] = [
    {
      ...
      author: "Admin",
      content:
        "¡Bienvenido al foro! Puedes compartir tus dudas o experiencias financieras aquí.",
      replies: [
  ...
  return (
    <div className="mx-auto max-w-2xl rounded-xl bg-black p-4 text-yellow-50 shadow-lg">
      ...
      <div className="mb-4 flex flex-col items-center justify-center gap-2 md:flex-row">
        <textarea
          rows={6}
          className="w-full flex-1 rounded border border-yellow-600 bg-zinc-900 p-2 text-white md:w-auto"
          placeholder="Escribe sobre un tema..."
          value={content}
          onChange={(e) => setContent(e.target.value)}
        />
        <button
          className="w-full rounded bg-blue-500 px-4 py-2 font-semibold text-white md:w-auto"
          onClick={addPost}
        >
          Publicar
        </button>
      ...
      <div className="space-y-6">
        {posts.map((post) => (
          <div
            key={post.id}
            className="rounded-lg border border-yellow-700 bg-zinc-900 p-3"
          >
            ...
            <div className="mb-2 wrap-break-word whitespace-pre-line text-white">
              {post.content}
            </div>
            <ReplySection
              post={post}
            ...
            {post.replies.length > 0 && (
              <div className="mt-2 border-l border-yellow-700 pl-4">
                {post.replies.map((reply) => (
                  <div
                    key={reply.id}
                    className="mb-2 flex items-center justify-between wrap-break-word"
                  >
                    <span>
      ...
      <AlertDialog
        open={showDeletePostDialog.open}
        onOpenChange={(open) =>
          setShowDeletePostDialog({ ...showDeletePostDialog, open })
        }
      >
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmarEliminarPublicacion}>
              Eliminar
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <AlertDialog
        open={showDeleteReplyDialog.open}
        onOpenChange={(open) =>
          setShowDeleteReplyDialog({ ...showDeleteReplyDialog, open })
        }
      >
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmarEliminarRespuesta}>
              Eliminar
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
...
const ReplySection: React.FC<{
  post: Post;
  addReply: (postId: number, author: string, content: string) => void;
  user: any;
  eliminarRespuesta: (postId: number, replyId: number) => void;
}> = ({ post, addReply, user, eliminarRespuesta }) => {
  ...
  return (
    <div className="mb-2 flex w-full items-center gap-2">
      <textarea
        rows={3}
        className="flex-1 rounded border border-yellow-600 bg-zinc-900 p-1 text-white"
        placeholder="Responder..."
        value={replyContent}
        onChange={(e) => setReplyContent(e.target.value)}
      />
      <button
        className="max-w-[150px] overflow-hidden rounded-lg bg-yellow-500 px-4 py-1 font-semibold text-ellipsis whitespace-nowrap text-black"
        onClick={() => {
