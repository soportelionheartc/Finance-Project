  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const portfolioId = req.body.portfolioId
        ? parseInt(req.body.portfolioId, 10)
        : null;
...
router.get("/:id", ensureAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    if (file.userId !== req.user!.id) {
      return res
        .status(403)
        .json({ error: "No tienes permiso para acceder a este archivo" });
    }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!assetId || !portfolioId) {
        return res
          .status(400)
          .json({ error: "Se requiere assetId y portfolioId" });
      }
      ...
      if (file.userId !== req.user!.id) {
        return res
          .status(403)
          .json({ error: "No tienes permiso para modificar este archivo" });
      }

      const updatedFile = await storage.updateFileAssetId(
        fileId,
        assetId,
        portfolioId,
      );
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const updatedFile = await storage.updateFilePortfolioId(
        fileId,
        portfolioId,
      );
      res.json({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (file.userId !== req.user!.id) {
        return res
          .status(403)
          .json({ error: "No tienes permiso para eliminar este archivo" });
      }
